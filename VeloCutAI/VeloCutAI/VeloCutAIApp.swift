...
final class EditorViewModel: ObservableObject {
    ...
    func playPause() {
        ...
        if player.rate == 0 {
            if currentTime < clip.trimStart || currentTime >= clip.trimEnd - 0.03 {
    ...
    func seek(by delta: Double) {
        guard let clip = selectedClip else { return }
        let value = min(max(clip.trimStart, currentTime + delta), clip.trimEnd)
    ...
    func setTrimStart(_ value: Double) {
        mutateSelectedClip { clip in clip.trimStart = min(max(0, value), max(0, clip.trimEnd - 0.05)) }
    ...
    func setTrimEnd(_ value: Double) {
        mutateSelectedClip { clip in clip.trimEnd = max(min(clip.duration, value), min(clip.duration, clip.trimStart + 0.05)) }
    ...
    func splitSelectedClip() {
        ...
        guard splitTime > clip.trimStart + 0.08, splitTime < clip.trimEnd - 0.08 else {
            errorMessage = "Поставьте курсор внутри клипа, а затем нажмите Split."
            return
        }
        registerUndo()
        var left = clip
        left.trimEnd = splitTime
    ...
    func resetColor() {
        registerUndo()
        brightness = 0; contrast = 1; saturation = 1; enhanceAmount = 0
        refreshPreview()
    }
    ...
    private func restore(_ snapshot: ProjectSnapshot) {
        clips = snapshot.clips; selectedClipID = snapshot.selectedClipID; overlayText = snapshot.overlayText
        overlayTextSize = snapshot.overlayTextSize; overlayTextY = snapshot.overlayTextY; selectedFilter = snapshot.selectedFilter
        brightness = snapshot.brightness; contrast = snapshot.contrast; saturation = snapshot.saturation; enhanceAmount = snapshot.enhanceAmount
    ...
    private static func applyLook(to source: CIImage, filter: EditorFilter, brightness: Double, contrast: Double, saturation: Double, enhanceAmount: Double) -> CIImage {
    ...
    private func makeExport() async throws -> URL {
        ...
        audioMix.inputParameters = [audioParams, musicParams].compactMap { $0 }

        let needsVC = selectedFilter != .original || abs(brightness) > 0.001 || abs(contrast - 1) > 0.001 || abs(saturation - 1) > 0.001 || enhanceAmount > 0.001 || !overlayText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
        var videoComposition: AVVideoComposition?
        if needsVC {
            let vc = AVVideoComposition(asset: composition) { [selectedFilter, brightness, contrast, saturation, enhanceAmount] request in
                request.finish(with: Self.applyLook(to: request.sourceImage, filter: selectedFilter, brightness: brightness, contrast: contrast, saturation: saturation, enhanceAmount: enhanceAmount), context: nil)
            }
            if !overlayText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                let renderSize = vc.renderSize
                let parentLayer = CALayer(); let videoLayer = CALayer()
                parentLayer.frame = CGRect(origin: .zero, size: renderSize); videoLayer.frame = parentLayer.frame; parentLayer.addSublayer(videoLayer)
                let textLayer = CATextLayer(); textLayer.string = overlayText; textLayer.alignmentMode = .center; textLayer.foregroundColor = UIColor.white.cgColor
                textLayer.shadowColor = UIColor.black.cgColor; textLayer.shadowOpacity = 0.8; textLayer.shadowRadius = 4; textLayer.shadowOffset = CGSize(width: 0, height: 2)
                textLayer.fontSize = CGFloat(overlayTextSize) * max(1, renderSize.width / 1080); textLayer.contentsScale = UIScreen.main.scale
                let height = max(80, textLayer.fontSize * 2.2)
                let y = max(0, min(renderSize.height - height, renderSize.height * CGFloat(1 - overlayTextY) - height / 2))
                textLayer.frame = CGRect(x: renderSize.width * 0.06, y: y, width: renderSize.width * 0.88, height: height); parentLayer.addSublayer(textLayer)
...
struct PlayerView: UIViewControllerRepresentable {
    ...
    func makeUIViewController(context: Context) -> AVPlayerViewController {
        let controller = AVPlayerViewController(); controller.player = player; controller.showsPlaybackControls = false; controller.videoGravity = .resizeAspect; return controller
    }
...
struct VideoPicker: UIViewControllerRepresentable {
    ...
    func makeUIViewController(context: Context) -> UIDocumentPickerViewController {
        ...
        controller.allowsMultipleSelection = true; controller.delegate = context.coordinator; return controller
    }
...
struct AudioPicker: UIViewControllerRepresentable {
    ...
    func makeCoordinator() -> Coordinator { Coordinator(onPick) }
    func makeUIViewController(context: Context) -> UIDocumentPickerViewController {
        let controller = UIDocumentPickerViewController(forOpeningContentTypes: [.audio], asCopy: true); controller.delegate = context.coordinator; return controller
    }
    func updateUIViewController(_ uiViewController: UIDocumentPickerViewController, context: Context) {}
    final class Coordinator: NSObject, UIDocumentPickerDelegate {
        let onPick: (URL) -> Void
...
struct EditorView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .sheet(item: $inspector) { tool in
                InspectorSheet(tool: tool, model: model).presentationDetents([.height(300), .medium, .large]).presentationDragIndicator(.visible).presentationBackground(.ultraThinMaterial)
            }
    ...
    private var topBar: some View {
        HStack(spacing: 12) {
            Button { showProjectInfo = true } label: {
                Image(systemName: "chevron.down").font(.headline.weight(.semibold)).frame(width: 38, height: 38).background(.thinMaterial, in: Circle())
            }
            VStack(alignment: .leading, spacing: 1) {
                Text("VeloCut").font(.headline.weight(.semibold))
                Text(model.clips.isEmpty ? "Новый проект" : "\(model.clips.count) клип. • \(format(model.projectDuration))").font(.caption2).foregroundStyle(.secondary)
            }
            Spacer()
            ...
            Button { inspector = .export } label: {
                Text("Экспорт").font(.subheadline.weight(.semibold)).padding(.horizontal, 14).frame(height: 36).background(Color.accentColor, in: Capsule()).foregroundStyle(.white)
            ...
            ZStack {
                ...
                if model.selectedClip != nil {
                    ...
                    if !model.overlayText.isEmpty {
                        Text(model.overlayText).font(.system(size: model.overlayTextSize, weight: .bold, design: .rounded)).multilineTextAlignment(.center).foregroundStyle(.white)
                            .shadow(color: .black.opacity(0.9), radius: 4, y: 2).padding(.horizontal, 24)
                            .position(x: geometry.size.width / 2, y: geometry.size.height * model.overlayTextY)
                    }
                } else {
                    Button { model.isImporting = true } label: {
                        VStack(spacing: 12) {
                            Image(systemName: "plus.rectangle.on.rectangle").font(.system(size: 40, weight: .medium))
                            Text("Добавить видео").font(.headline)
                            Text("Можно выбрать несколько MP4 или MOV").font(.caption).foregroundStyle(.secondary)
                        }.foregroundStyle(.white)
                    }.buttonStyle(.plain)
                }
            }
            .overlay(alignment: .topTrailing) {
                if model.selectedFilter != .original || model.enhanceAmount > 0 {
                    HStack(spacing: 5) { Image(systemName: "wand.and.stars"); Text(model.selectedFilter.rawValue) }
                        .font(.caption2.weight(.semibold)).padding(.horizontal, 9).padding(.vertical, 6).background(.ultraThinMaterial, in: Capsule()).padding(10)
                }
            }
    ...
    private var playbackBar: some View {
        HStack(spacing: 18) {
            Spacer()
            Button { model.seek(by: -1) } label: { Image(systemName: "gobackward.10") }
            ...
            Text(selectedTimeLabel).font(.caption.monospacedDigit()).foregroundStyle(.secondary).frame(width: 92, alignment: .leading)
            Spacer()
    ...
    private var timelineArea: some View {
        VStack(spacing: 8) {
            HStack {
                Label("Таймлайн", systemImage: "timeline.selection").font(.caption.weight(.semibold)); Spacer()
                Button { model.timelineZoom = max(0.7, model.timelineZoom - 0.25) } label: { Image(systemName: "minus.magnifyingglass") }
                Button { model.timelineZoom = min(2.5, model.timelineZoom + 0.25) } label: { Image(systemName: "plus.magnifyingglass") }
            }.foregroundStyle(.secondary).padding(.horizontal, 16)
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 5) {
                    ForEach(Array(model.clips.enumerated()), id: \.element.id) { index, clip in
                        TimelineClipView(clip: clip, index: index, selected: clip.id == model.selectedClipID, zoom: model.timelineZoom).onTapGesture { model.selectClip(clip.id) }
                    }
                    Button { model.isImporting = true } label: {
                        Image(systemName: "plus").font(.title3.weight(.semibold)).frame(width: 44, height: 54).background(.thinMaterial, in: RoundedRectangle(cornerRadius: 12, style: .continuous))
                    }
                }.padding(.horizontal, 14).padding(.vertical, 5)
            }
            if model.selectedClip != nil {
                HStack(spacing: 8) {
                    compactAction("chevron.left", "Назад") { model.moveSelectedClip(-1) }
                    compactAction("scissors", "Split") { model.splitSelectedClip() }
                    compactAction("plus.square.on.square", "Копия") { model.duplicateSelectedClip() }
                    compactAction("chevron.right", "Вперёд") { model.moveSelectedClip(1) }
                    compactAction("trash", "Удалить", role: .destructive) { model.deleteSelectedClip() }
    ...
    private var bottomToolBar: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 5) {
                editorTool("scissors", "Обрезка", .trim); editorTool("speedometer", "Скорость", .speed); editorTool("waveform", "Аудио", .audio)
                editorTool("textformat", "Текст", .text); editorTool("camera.filters", "Фильтры", .filters); editorTool("slider.horizontal.3", "Настройка", .adjust); editorTool("wand.and.stars", "Улучшить", .enhance)
            }.padding(.horizontal, 10)
        }.padding(.vertical, 8).background(.ultraThinMaterial).disabled(model.clips.isEmpty)
    }
    ...
    private var exportOverlay: some View {
        ZStack {
            ...
            }.padding(24).background(.ultraThickMaterial, in: RoundedRectangle(cornerRadius: 24, style: .continuous))
        }
    }

    private var selectedTimeLabel: String {
        guard let clip = model.selectedClip else { return "0:00 / 0:00" }
        return "\(format(max(0, model.currentTime - clip.trimStart))) / \(format(clip.sourceDuration))"
    }
    ...
    private func editorTool(_ icon: String, _ title: String, _ tool: InspectorTool) -> some View {
        Button { inspector = tool; model.haptic(.selection) } label: {
            VStack(spacing: 5) { Image(systemName: icon).font(.system(size: 17, weight: .medium)).frame(height: 22); Text(title).font(.caption2) }
                .frame(width: 72, height: 52).contentShape(Rectangle())
        }.buttonStyle(.plain)
    }

    private func compactAction(_ icon: String, _ title: String, role: ButtonRole? = nil, action: @escaping () -> Void) -> some View {
        Button(role: role, action: action) {
            VStack(spacing: 3) { Image(systemName: icon); Text(title).font(.system(size: 9)) }
                .frame(maxWidth: .infinity).frame(height: 42).background(.thinMaterial, in: RoundedRectangle(cornerRadius: 10, style: .continuous))
        }.buttonStyle(.plain)
    }

    private func format(_ seconds: Double) -> String {
        let value = max(0, Int(seconds.rounded())); return String(format: "%d:%02d", value / 60, value % 60)
    }
}
...
struct TimelineClipView: View {
    ...
    var body: some View {
        ...
        ZStack {
            ...
            HStack(spacing: 3) {
                ForEach(0..<max(2, Int(width / 26)), id: \.self) { _ in
                    RoundedRectangle(cornerRadius: 4).fill(Color.secondary.opacity(0.22)).overlay { Image(systemName: "play.rectangle.fill").font(.caption2).foregroundStyle(.secondary) }
                }
            }.padding(5)
            VStack {
                HStack { Text("\(index + 1)").font(.system(size: 9, weight: .bold)).padding(.horizontal, 5).padding(.vertical, 3).background(.ultraThinMaterial, in: Capsule()); Spacer(); Text("\(clip.speed, specifier: "%g")×").font(.system(size: 9, weight: .semibold)) }
                Spacer()
                HStack { Text(clip.name).font(.system(size: 9, weight: .medium)).lineLimit(1); Spacer(); Text(format(clip.outputDuration)).font(.system(size: 9).monospacedDigit()) }
            }.padding(6)
    ...
    private func format(_ seconds: Double) -> String { let value = max(0, Int(seconds.rounded())); return String(format: "%d:%02d", value / 60, value % 60) }
}
...
struct InspectorSheet: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            }.padding(.horizontal, 18).navigationTitle(title).navigationBarTitleDisplayMode(.inline)
    ...
    private var title: String {
        switch tool {
        case .trim: return "Обрезка"; case .speed: return "Скорость"; case .audio: return "Аудио"; case .text: return "Текст"
        case .filters: return "Фильтры"; case .adjust: return "Настройка"; case .enhance: return "Улучшение"; case .export: return "Экспорт"
        }
    }
    ...
    private var trimView: some View {
        VStack(spacing: 22) {
            if let clip = model.selectedClip {
                ...
                HStack { Label("После обрезки", systemImage: "clock"); Spacer(); Text(time(clip.outputDuration)).monospacedDigit() }.font(.subheadline).foregroundStyle(.secondary)
                Button { model.splitSelectedClip() } label: { Label("Разделить по текущему курсору", systemImage: "scissors").frame(maxWidth: .infinity) }.buttonStyle(.borderedProminent)
            }
            Spacer()
        }.padding(.top, 18)
    }
        ...
        VStack(spacing: 20) {
            ...
            HStack {
                ForEach([0.25, 0.5, 1.0, 1.5, 2.0, 3.0, 5.0], id: \.self) { value in
                    Button("\(value, specifier: "%g")×") { model.setSpeed(value) }.font(.caption.weight(.semibold)).buttonStyle(.bordered).tint(abs((model.selectedClip?.speed ?? 1) - value) < 0.001 ? .accentColor : .secondary)
                }
            }
    ...
    private var audioView: some View {
        VStack(spacing: 20) {
            ...
            Divider()
            HStack {
                VStack(alignment: .leading, spacing: 3) { Text("Музыка").font(.headline); Text(model.musicName ?? "Не добавлена").font(.caption).foregroundStyle(.secondary).lineLimit(1) }
                Spacer(); Button(model.musicURL == nil ? "Добавить" : "Заменить") { model.isAudioImporting = true }.buttonStyle(.borderedProminent)
            }
            if model.musicURL != nil { valueSlider(title: "Громкость музыки", value: $model.musicVolume, range: 0...1.5, suffix: "\(Int(model.musicVolume * 100))%") }
            Spacer()
        }.padding(.top, 18)
    }
    ...
    private var textView: some View {
        VStack(spacing: 18) {
            TextField("Введите текст", text: $model.overlayText, axis: .vertical).textFieldStyle(.roundedBorder).lineLimit(1...3).onTapGesture { model.registerUndo() }
            ...
            HStack { ForEach([(0.2, "Верх"), (0.5, "Центр"), (0.82, "Низ")], id: \.0) { item in Button(item.1) { model.overlayTextY = item.0 }.buttonStyle(.bordered) } }
            Spacer()
        }.padding(.top, 18)
    }
            ...
            HStack(spacing: 12) {
                ForEach(EditorFilter.allCases) { filter in
                    Button { model.setFilter(filter) } label: {
                        VStack(spacing: 8) {
                            RoundedRectangle(cornerRadius: 16, style: .continuous).fill(.thinMaterial).frame(width: 86, height: 86)
    ...
    private var adjustView: some View {
        VStack(spacing: 20) {
            ...
            Button("Сбросить настройки") { model.resetColor() }.buttonStyle(.bordered); Spacer()
        }.padding(.top, 18)
    }
    ...
    private var enhanceView: some View {
        VStack(spacing: 20) {
            ...
            Text("Шумоподавление, повышение резкости и восстановление деталей выполняются при предпросмотре и экспорте.").font(.caption).foregroundStyle(.secondary).multilineTextAlignment(.center)
    ...
    private var exportView: some View {
        VStack(spacing: 18) {
            Picker("Качество", selection: $model.exportQuality) { ForEach(ExportQuality.allCases) { quality in Text(quality.rawValue).tag(quality) } }.pickerStyle(.segmented)
            VStack(spacing: 12) {
                exportRow("Длительность", time(model.projectDuration)); exportRow("Клипы", "\(model.clips.count)"); exportRow("Фильтр", model.selectedFilter.rawValue)
                exportRow("Текст", model.overlayText.isEmpty ? "Нет" : "Да"); exportRow("Музыка", model.musicURL == nil ? "Нет" : "Да")
            ...
            Button { dismiss(); model.exportProject() } label: {
                Label("Экспортировать \(model.exportQuality.rawValue)", systemImage: "square.and.arrow.up").font(.headline).frame(maxWidth: .infinity).frame(height: 48)
            }.buttonStyle(.borderedProminent).disabled(model.isExporting || model.clips.isEmpty)
            Spacer()
        }.padding(.top, 18)
    }
    ...
    private func valueSlider(title: String, value: Binding<Double>, range: ClosedRange<Double>, suffix: String) -> some View {
        VStack(spacing: 8) {
            HStack { Text(title).font(.subheadline.weight(.medium)); Spacer(); if !suffix.isEmpty { Text(suffix).font(.caption.monospacedDigit()).foregroundStyle(.secondary) } }
    ...
    private func time(_ seconds: Double) -> String { let value = max(0, Int(seconds.rounded())); return String(format: "%d:%02d", value / 60, value % 60) }
}
...
struct ProjectInfoSheet: View {
    ...
    var body: some View {
        NavigationStack {
            List {
                Section("Проект") { LabeledContent("Клипы", value: "\(model.clips.count)"); LabeledContent("Длительность", value: format(model.projectDuration)); LabeledContent("Качество экспорта", value: model.exportQuality.rawValue) }
                Section("VeloCut") { Label("Нативный интерфейс SwiftUI", systemImage: "iphone"); Label("Монтаж и экспорт AVFoundation", systemImage: "film"); Label("Фильтры Core Image", systemImage: "camera.filters") }
            }.navigationTitle("Проект").navigationBarTitleDisplayMode(.inline).toolbar { ToolbarItem(placement: .confirmationAction) { Button("Готово") { dismiss() } } }
        }
    }
    private func format(_ seconds: Double) -> String { let value = max(0, Int(seconds.rounded())); return String(format: "%d:%02d", value / 60, value % 60) }
}
